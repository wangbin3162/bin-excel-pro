  ...
  for (let borderInfo of sheet.config.borderInfo) {
    if (borderInfo.rangeType === 'cell') {
      ...
      if (borderInfo.value.r) {
        ...
        if (
          newSheet.cellData[rowIndex] &&
          newSheet.cellData[rowIndex][Number(colIndex) + 1]?.s?.bd?.l
        ) {
      ...
      if (borderInfo.value.b) {
        ...
        if (
          ...
          newSheet.cellData[rowIndex][colIndex]?.s?.bd?.t
        ) {
          delete newSheet.cellData[rowIndex][colIndex - 1]?.s?.bd?.t
        }
      }

      newSheet.cellData[rowIndex][colIndex].s.bd = Object.assign(
        newSheet.cellData[rowIndex][colIndex].s.bd || {},
        newBorder,
      )
          ...
          case 'border-left':
            for (let r = startRow; r <= endRow; r++) {
              for (let c = startColumn; c < startColumn + 1; c++) {
                ...
                newSheet.cellData[r][c].s.bd = Object.assign(
                  newSheet.cellData[r][c].s.bd || {},
                  newBorder,
                )
              }
            }
